...
---@class Game341Hub extends GameProtocolHub
local Game341Hub = class("Game341Hub", super)
-- Game341Hub.ResumeState = {
--     Normal = 1,
--     Free = 2,
--     Special = 3,
-- }
function Game341Hub:parseNormalSpinRet(aData)
...
function Game341Hub:parseSpecialData(aData)
    ...
    if (not aData.total_special_win or aData.total_special_win == 0) and aData.win > 0 then -- 341 的 total_special_win 就是win 服务器说的
        aData.total_special_win = aData.win
    end


    specialSpin.bet = 0;

    specialSpin.winCoin = aData.total_win + aData.enter_win
...
function Game341Hub:parseResumeData(aEnter, aEnterData, aResume)
...
function Game341Hub:parseFreeSpinRet(aData)
...
-- function Game341Hub:parseFreeSpinType(aData)
...
function Game341Hub:parseGrids(aData)
    local result = {}
...
function Game341Hub:parseGrid(aData)
    return {
        index = aData.index,
        icon = aData.icon,
...
function Game341Hub:parseSpin(aData)
    local spin = {
...
function Game341Hub:calcLevelUpMoney(money,level_ups)
