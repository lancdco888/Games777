function luaj_entry_init()
    local luaj_class_name = "org/cocos2dx/lua/Interface"
    local entry_mp = {}
    local found_mp = false

    local fu = cc.FileUtils:getInstance()
    local df = fu:getDefaultResourceRootPath() .. "src/"
    ...
    for _,f in ipairs(files_) do
        if string.find(f, ".mp") then
            ...
            local success, data_ = pcall(json.decode, str_)
            if not success then
                -- 尝试使用二进制解码
                print("not a asscii .mp file, try binary.")
                ...
                for i=3,string.len(str_) do
                    local val = bit.bxor(string.byte(str_, i, i), key)
                    dst = dst .. string.char(val)
                end

                success, data_ = pcall(json.decode, dst)
                if not success then
    ...
    if platform == cc.PLATFORM_OS_ANDROID then
        ...
        local old_callStaticMethod = luaj.callStaticMethod
        luaj.callStaticMethod = function(__, methodName, args, sig)
            local new_method_name = entry_mp[methodName]
            if new_method_name == nil then new_method_name = methodName end
            return old_callStaticMethod(luaj_class_name, new_method_name, args, sig)
        end

        local old_checkStaticMethod = LuaJavaBridge.checkStaticMethod
        LuaJavaBridge.checkStaticMethod = function(className, methodName, sig)
            local new_method_name = entry_mp[methodName]
            if new_method_name == nil then new_method_name = methodName end
            print("checkStaticMethod:" .. methodName .. " ==>>  " .. new_method_name)
            return old_checkStaticMethod(luaj_class_name, new_method_name, sig)
        end
    end
end
