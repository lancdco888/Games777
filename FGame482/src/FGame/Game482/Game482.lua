...
local Game482 = Class("Game482", BaseGame)

function Game482:ctor(render)
    self.render = render
...
function Game482:GetEffectLayerChildren()
    local res = {}
...
-- function Game482:ResetScoreLabel()
...
function Game482:PlayZeusMultiNotifyAnim()
    print("Game482:PlayZeusMultiNotifyAnim >>> ")
    Utils.PlaySound("ui://Game482/multiple_"..math.random(1,10))
    local zeusMultiNotify = self.render:GetTransition("zeus-multi-notify")
    zeusMultiNotify:Play()
end

function Game482:PlayZeusMultiNotifyEndAnim()
    print("Game482:PlayZeusMultiNotifyEndAnim >>> ")
    local zeusMultiNotify = self.render:GetTransition("zeus-multi-notify-end")
    zeusMultiNotify:Play()
end


function Game482:SetScoreLabelView(val,duration)
    print("Game482:SetScoreLabelView >>> ")
    Utils.ScrollBottomWinMoneyTo(val, durantion)
end


function Game482:ResetFreeRemainCountView()
    self.freeRemainCountLabel.visible = true
end

Game482.curSpinAccumulatedWin = 0
Game482.curAccumulatedWinInFree = 0


function Game482:__delete()
    self:Clear()
...
-- @interface
-- @brief update
function Game482:Update(dt)
    Game482.super.Update(self, dt)
...
function Game482:Clear()
    FCasinoCtx.commonPanel:ShowTop(true, false)
    self:StopDelayCallSpin()
end

function Game482:SetTotalWin(value)
    self:SetWinMoney(value,false)
end

-- 当前spin的赢钱
function Game482:ShowTotalWin(curWinCoin, multiCount, callback)
    ...
    local function delayNext(time)
        print("Game482:ShowTotalWin >>> delayNext >>> time >>> "..time)
    ...
    if multiCount >= 15 then
        ...
        -- Utils.ScrollBottomWinMoneyTo(curWinCoin,.2,function()
            self.BigWinAnim:PlayScoreAnim(curWinCoin, function()
                print("Game482:ShowTotalWin >>> self.BigWinAnim:PlayScoreAnim >>> Complete")
...
function Game482:SetFreeRemainCountLabel(count)
    self:ResetFreeRemainCountView()
    self.freeRemainCountLabel.text = tostring(count)
end


function Game482:UpdateFreeRemainCountLabel()
...
-- @brief 弹出
function Game482:ShowEnterFreePanel(addFreeTime, isFreeInFree)
    self.frameWaitSoundHandler = Utils.PlaySound("ui://Game482/frame_wait")
    FCasinoCtx:SetSpinStatus(FSpinStatus.WAITING)

    if not self.enterFreePanel then
        self.enterFreePanel = FairyGUI.UIPackage.CreateObject("Game482", "EnterFreePanel")
    ...
    local function onStartBtnClick(btn)
        ...
        Utils.PlaySound("ui://Game482/Free_Start_Button")
...
function Game482:ShowFreeTotalSettlement(freeTotalWinCoin)
    self.frameWaitSoundHandler = Utils.PlaySound("ui://Game482/freegame_end")
    ...
    if not self.result then
        self.result = FairyGUI.UIPackage.CreateObject("Game482", "FreeGameResult")
    ...
    Utils.PlaySound("ui://Game482/bgm_totalwin_end")
    ...
    self.soundTotalWin = Utils.PlaySound("ui://Game482/bgm_totalwin_main")

end


function Game482:SetGameMode(mode)
    FCasinoCtx:SetGameMode(mode)
    self.curSlotType = mode

    local bgmUrl = mode == FGameMode.FREE and "ui://Game482/fg_free" or "ui://Game482/fg_normal"
...
function Game482:SetWinMoney(value,rolling)
...
-- @brief 在游戏中断线重连恢复游戏
-- @param msg 断线重连数据
function Game482:ReconnectRecoveryGame(msg, isInitialize)
    print("Game482:ReconnectRecoveryGame >>> ")
    -- 断线重连标记
    self.isReconnect = true
    -- dump(msg,"msg",10)
    local currentWinCoin = msg.currentWinCoin
    -- print("Game482:ReconnectRecoveryGame>>>>>>>>> currentWinCoin  "..currentWinCoin)
    self:SetTotalWin(currentWinCoin)
    if msg.status == 1 then
        print("Game482:ReconnectRecoveryGame >>> msg.status == 1")
        ...
        if self.isRecoveryIntoFree then
            ...
            FToolSet.PlayBGM("ui://Game482/bgm_fs")
            
            FCasinoCtx:SetSpinStatus(FSpinStatus.SPIN)
            self:AutoClick()
        else
            self:OnNormalSpinResult(msg.resumedNormal.normalSpin)
            FToolSet.PlayBGM("ui://Game482/bgm_mg")
        end
    elseif msg.status == 2 then
        print("Game482:ReconnectRecoveryGame >>> msg.status == 2")
...
-- @interface
-- @brief 点击开始按钮
function Game482:OnClickSpin()
    print("Game482:OnClickSpin >>> ")
    -- Check can spin
    local canSpin = true
    ...
    Utils.PlaySound("ui://Game482/start")
...
function Game482:OnClickStop()
    print("Game482:OnClickStop >>> ")
    -- 普通游戏状态点击停止按钮
    if FCasinoCtx.curGameMode == FGameMode.NORMAL then
...
-- @brief 处理普通旋转结果
function Game482:OnNormalSpinResult(spinData)
    ...
    local doWhenSetReelSymbolCompelte = function()
        ...
        if isIntoFree then
            Utils.PlaySound("ui://Game482/free_spin")
            -- 显示进免费,sc动画
            ...
            self.enterDelayTimer = StartOnceTimer(function()
                -- self:SetGameMode(FGameMode.FREE)
                Utils.PlaySound("ui://Game482/frame_appear")
...
Game482.StuckedFnTable = {}
Game482.curFlowAniPlayingCount = 0
function Game482:DoesFlowAniPlayComplete()
    return Game482.curFlowAniPlayingCount == 0
end
function Game482:FlowAniCountAdd()
    print("Game482:FlowAniCountAdd >>> "..tostring(Game482.curFlowAniPlayingCount))
    Game482.curFlowAniPlayingCount = Game482.curFlowAniPlayingCount + 1
end
function Game482:FlowAniCountSub()
    print("Game482:FlowAniCountSub >>> "..tostring(Game482.curFlowAniPlayingCount))
    Game482.curFlowAniPlayingCount = Game482.curFlowAniPlayingCount - 1
    if Game482.curFlowAniPlayingCount < 0 then Game482.curFlowAniPlayingCount = 0 end
    if Game482.curFlowAniPlayingCount == 0 then
        self:StuckedFnTableDo()
    end
end
function Game482:StuckedFnTableAdd(fn)
    print("Game482:StuckedFnTableAdd >>>>>>> ")
    table.insert(Game482.StuckedFnTable, fn)
end
function Game482:StuckedFnTableDo()
    print("Game482:StuckedFnTableDo >>>>>>>>")
    for _, fn in ipairs(Game482.StuckedFnTable) do
        fn()
    end
end
function Game482:ResetFlowAniData()
    print("Game482:ResetFlowAniData >>>>>>>>".. tostring(#Game482.StuckedFnTable))
    for _, fn in ipairs(Game482.StuckedFnTable) do
        fn = nil
    end
    Game482.StuckedFnTable = {}
...
-- @brief 处理免费游戏旋转结果
function Game482:OnFreeSpinResult(spinData)
    ...
    self.slotContainer[self.curSlotType]:SetReelSymbolData(spinData, isReconnect, function()
        print("Game482:OnFreeSpinResult >>> self.slotContainer[self.curSlotType]:SetReelSymbolData >>> Complete")
        function doWhenSetReelSymbolCompelte()
            print("Game482:OnFreeSpinResult >>> doWhenSetReelSymbolCompelte >>> ")
            ...
            if isFreeInFree then
                print("Game482:OnFreeSpinResult >>> isFreeInFree >>> true")
            ...
            elseif remainFreeTime == 0 then
                print("Game482:OnFreeSpinResult >>> remainFreeTime == 0 >>> true")

                -- self.curSCGridsIndexArr = {}
                -- 免费结束
                if not isReconnect then
                    print("Game482:OnFreeSpinResult >>> remainFreeTime == 0 >>> not isReconnect >>> true")
                    ...
                    print("Game482:OnFreeSpinResult >>> remainFreeTime == 0 >>> isReconnect >>> true")
                    FCasinoCtx:GetGame().curAccumulatedWinInFree = 0
                    FCasinoCtx:GetGame().totalMultiplierCount = 0
                    self.totalMultiplierLabel.text = ""
                    self.totalMultiplierLabel.visible = true
        ...
        if self:DoesFlowAniPlayComplete() then
            print("Game482:OnFreeSpinResult >>> self:DoesFlowAniPlayComplete() >>> true")
            doWhenSetReelSymbolCompelte()
        else
            print("Game482:OnFreeSpinResult >>> self:StuckedFnTableAdd(doWhenSetReelSymbolCompelte) >>> ")
            self:StuckedFnTableAdd(doWhenSetReelSymbolCompelte)
        end
...
function Game482:StopDelayCallSpin()
...
function Game482:DelayCallSpin(delay)
    self:StopDelayCallSpin()
...
function Game482:ResetAwardHistoryView()
    self.awardsHistoryPanel:ResetListView()
end

function Game482:PlayAwardHistoryItemAni()
    -- if true then return end
    self.awardsHistoryPanel:ShowNewItemAnimation()
end

function Game482:SetCurAwardsData(data)
    self.awardsHistoryPanel:SetCurAwardsData(data)

end

function Game482:UpdateMultiCountLabel(curTotalMultiplierCount)
