...
-- 移动一次的时间
local MoveSecs = 1.3333333333333333
-- 一次移动需要的时间
...
local crabActions = {}
local crabBallAction
local carbBallActionCnt = 2
...
local CollisionChecks = function()
    ...
    --print("hit fish:", cannonId, bulletId)
...
lua:Set_onInit(function()
    ...
    if not FF_G.IsServer then
        ...
        do
            ...
            local panel = FF_G.LoadLuaFunc("script/panel/wakeup_kingcrab.lua")(this, lua, root)
            panel.Init(parent)
            ...
            do
                ...
                local ret, action = root:ShowEffect("ext/juexingbawangxie_carbball/CarbBall.actions",
                        "carbball", "", FF_G.kNodeIndex_Top, 0, 0, false)
                assert(ret)
                crabBallAction = action
...
local UpdateBoatPos = function()
    if not FF_G.IsServer then
        ...
        local animSecs = 0.0
        if useTime <= MoveFrontSecs then
            percent = useTime / MoveFrontSecs
            animName = "move"
            animSecs = math.fmod(useTime, MoveSecs)
        elseif useTime >= MoveTailSecs then
            percent = 1.0 - (useTime - MoveTailSecs) / MoveFrontSecs
            animName = "move"
            animSecs =  math.fmod(useTime - MoveTailSecs, MoveSecs)
        else
            animSecs = math.fmod(useTime - MoveFrontSecs, AttackSecs)
        end

        --print("secs:", actionSecs)
        for i = 1, carbBallActionCnt do
            local pos = FF_G.GetPercentPos(startPos[i], destPos[i], percent)
            local action = crabActions[i]
            local anim = action:GetAnim()
            anim:SetAnim(animName)
            anim:SetElapsedSeconds(animSecs)
            --action:SetAction(actionName)
            --action:SetElapsedSeconds(actionSecs)
            anim:SetPos(pos.x, pos.y)
        end
    end
end
...
lua:Set_onUpdate(function(dt)
    ...
    if not FF_G.IsServer then
        ...
        if uiPanel.SetValue(t.value) then
            local node = FF_G_Client.GetActionRootNode(crabBallAction)
            assert(node)
