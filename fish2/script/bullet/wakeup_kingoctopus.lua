...
lua:Set_onInit(function()
    if not FF_G.IsServer then
        local x, y = 0, math.min(1000, -width / 2)
        for i = 1, carbBallActionCnt do
            local x1, y1 = root:Rotate(x, y, angles[i])
            startPos[i] = {
                x = x1,
                y = y1,
            }
    ...
    if not FF_G.IsServer then
        ...
        do
            ...
            for i = 1, carbBallActionCnt do
                local ret, action = root:ShowEffect("spine/AwakenKingTaco.actions",
        ...
        do
            ...
            local panel = FF_G.LoadLuaFunc("script/panel/wakeup_kingoctopus.lua")(this, lua, root)
            panel.Init(parent)
...
lua:Set_onUpdate(function(dt)
    ...
    if FF_G.IsServer then
        if t.state == State_BoatMove then
            -- 触发爆炸
            if not t.hasBomb and t.timeLeft <= BoatMoveSecs / 5 * 4 then
                CollisionChecks()
                t.hasBomb = true
            end
        end
