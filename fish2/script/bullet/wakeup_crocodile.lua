...
lua:Set_onInit(function()
    ...
    if not FF_G.IsServer then
        ...
        do
            ...
            local panel = FF_G.LoadLuaFunc("script/panel/wakeup_crocodile.lua")(this, lua, root)
            panel.Init(parent)
            ...
            do
                ...
                local ret, action = root:ShowEffect("ext/WakeupCrocodileBall/WakeupCrocodileBall.actions",
                ...
                --action:GetAnim():SetScale(0.5, 0.5)
...
local UpdateBoatPos = function()
    if not FF_G.IsServer then
        ...
        for i = 1, carbBallActionCnt do
            ...
            local anim = action:GetAnim()
            anim:SetPos(pos.x, pos.y)
            anim:SetElapsedSeconds(useTime)
        end
    end
end
...
--local frameDt = 1 / 10.0

lua:Set_onUpdate(function(dt)
    t.leftUpdateSecs = t.leftUpdateSecs + dt
    while t.leftUpdateSecs >= frameDt
    do
        if updateFrame(frameDt) ~= 0 then
            return 1
        end
        t.leftUpdateSecs = t.leftUpdateSecs - frameDt
    end
    ...
    if not FF_G.IsServer then
        ...
        if t.state == State_BoatMove then
            UpdateBoatPos()
            if BoatMoveSecs - t.timeLeft > 0.15 and t.timeLeft > 0.6 then
                showActions = true
            end
        end
        --for _, action in ipairs(crabActions) do
        for i = 1, carbBallActionCnt do
            local action = crabActions[i]
            local isLeft = i % 2 == 1
            local show = isLeft == t.isLeft
            action:GetAnim():SetVisible(show and showActions)
        end
        UpdateUiPos()
