...
lua:Set_onInit(function()
    --print("zhadanxie bullet init.", t.ratio, t.bulletCount, t.isMultiple)
    if not t.hideZhadanxie then
        if t.isMultiple then
            anim:SetAnim("over_multiple")
        else
            anim:SetAnim("die")
        end
    end
    ...
    if not FF_G.IsServer then
        ...
        if t.isMultiple then
            bgImg = cc.Sprite:createWithSpriteFrameName("lianhuanzhadanxie_baozhacishuxianshi.png")
            local size = bgImg:getContentSize()

            timesTxt = FF_G_Client.CreateNumText("lianhuanzhadanxie_shuzi_%d.png", 25, 32)
            timesTxt.SetString("" .. (t.curTimes + 1))
            timesTxt.node:setPosition(size.width / 2, size.height / 2)

            if root:IsRotate() then
                timesTxt.node:setScale(-1, -1)
            end

            bgImg:addChild(timesTxt.node)
            bgImg:setScale(0.75)
            node:addChild(bgImg)
        end
...
lua:Set_onUpdate(function(dt)
    ...
    if t.curTimes >= t.totalTimes then
        ...
        if t.waitTime <= 0 then
            if t.value > 0 then
                --local fishTypeId = t.isMultiple and 203 or 202
    ...
    if time <= 0 then
        ...
        local ret, action = root:ShowEffect("actions/special/zhadanxie.actions", "boom",
                "", FF_G.kNodeIndex_EffectTop, destPos[1], destPos[2], true)
        assert(ret)
        local anim = action:GetAnim()
        anim:SetScale(2.5, 2.5)
        if FF_G.IsServer then
            local fish = anim:CollisionChecksFish()
            local destFish = {}
            for _, fish1 in ipairs(fish) do
                if fish1:IsNormalFish() or fish1:IsGoldenFish() or
                        FF_G.IsCSNormalFish(fish1:GetTypeId()) then
            ...
            if #destFish > 0 then
                ...
                --print("zhadanxie hitFish, use bullet:", useCount, #destFish, cannonId, id)
            end
        end
...
lua:Set_onFishDeath(function(fishId, relationFish)
    ...
    for _, fishId1 in ipairs(relationFish) do
        --print("zhadanxie:fish death:", fishId1, ratio)
