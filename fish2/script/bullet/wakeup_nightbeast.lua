...
local BoatMoveSecs = 4.8
local ServerCdCheckSecs = 4.35
-- 一次移动显示爆炸效果的次数
...
lua:Set_onInit(function()
    ...
    if not FF_G.IsServer then
        ...
        do
            ...
            for i = 1, carbBallActionCnt do
                local ret, action = root:ShowEffect("spine/AwakenKingAnglerFish.actions",
        ...
        do
            ...
            local panel = FF_G.LoadLuaFunc("script/panel/wakeup_nightbeast.lua")(this, lua, root)
            panel.Init(parent)
            ...
            do
                local actionParent = cc.Node:create()
                parent:addChild(actionParent)
                do
                    local ret, action = root:ShowEffect("ext/fish_jxayjs_boomstar/fish_jxayjs_boomstar.actions",
                            "effect2", "", FF_G.kNodeIndex_Top, 0, 0, false)
                    assert(ret)
                    local node = FF_G_Client.GetActionRootNode(action)
                    FF_G_Client.SwitchParentTo(node, actionParent)
                    effectAction = action
                end
                local ret, action = root:ShowEffect("ext/fish_jxayjs_boomstar/fish_jxayjs_boomstar.actions",
                        "effect3", "", FF_G.kNodeIndex_Top, 0, 0, false)
                assert(ret)
                ballAction = action
                local node = FF_G_Client.GetActionRootNode(action)
                FF_G_Client.SwitchParentTo(node, actionParent)
                node:runAction(cc.RepeatForever:create(cc.RotateBy:create(8.0, 360)));
                --cc.SpriteFrameCache:getInstance():addSpriteFrames("ui/octopus_ball.plist")
                --local node = FF_G_Client.CreateFrameAnim("OctopusBall_%02d.png", 1, 18, 1 / 30)
...
local UpdateBoatPos = function()
    if not FF_G.IsServer then
        local percent = 1.0
        if t.bombLeftTimes == t.totalBombTimes then
            local moveSecs = 1
            local useTime = math.min(BoatMoveSecs - t.timeLeft, moveSecs)
            percent = useTime / moveSecs
        elseif t.bombLeftTimes == 1 then
            local moveSecs = 0.3
            if t.timeLeft < moveSecs then
                percent = 1 - (moveSecs - t.timeLeft) / moveSecs
            end
        end
        for i = 1, carbBallActionCnt do
            local pos = FF_G.GetPercentPos(startPos[i], destPos[i], percent)
            local action = crabActions[i]
            local anim = action:GetAnim()
            anim:SetPos(pos.x, pos.y)
        end
