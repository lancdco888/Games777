...
local Init = function()
    if FF_G_Client then
        FF_G_Client.AddCombinedBg(this, 1.25, {40, 0})
        for _, fish in ipairs(this:GetChildren()) do
            FF_G_Client.AddCombinedBg(fish, 1.25, {40, 0})
        end
    end
end
...
lua:Set_onInit(function()
    if not t.inited then
        ...
        for _, offset in ipairs(offsets) do
            local childFish = root:CreateFish("actions/general/shark.actions", "fish0.lua")
