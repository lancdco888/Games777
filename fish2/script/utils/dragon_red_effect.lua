...
if not FF_G.IsServer then
    ...
    local PushEffect = function(x, y, scaleX, scaleY, angle)
        local ret, action = root:ShowEffect("ext/dragonred_fire/dragonred.actions", "screen_fire",
    ...
    -- 冥焰赤龙火焰效果
    FF_G_Client.ShowDragonRedEdgeFireEffect = function()
        ref = ref + 1
        if ref > 1 then return end
        musicIndex = FF_G.playEffect("RedDragon_OutFire", true)
        local offX = {0, -527, 527}
        for i = 1, 3 do
    ...
    FF_G_Client.CloseDragonRedEdgeFireEffect = function()
        ref = ref - 1
