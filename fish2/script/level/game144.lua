...
local InitTimeline = function()
    ...
    --场景1：--雷霸龙215 场景7  招财宝蟾 327 场景15
    do
     
        local scene = scene1
        scene.PushFishGroups(fishGroup)
        scene.AddSwitchBgEvent(0, 15)   -- 场景
        scene.AddSwitchBgMusicEvent(1, "tournament_waiting")
        -- 持续时间段定义    
            local t1 = 60
            local t2 = 280
            local t3 = 120
        ...
        -- 第二段场景时间划分    
            ...
            scene.AddBossWarningEvent(t1+t2+t3 +2, FF_G.BossWarning_BossCome, {fishTypeId = 333})
            scene.AddSwitchBgMusicEvent(t1+t2+t3+2.8, "bossstage") --下个场景音效
            scene.AddSwitchBgEvent(t1+t2+t3 +4.5, 15) -- 场景
        ...
        -- 出彩金鱼：SetFishGroup("goldedfish", {320,321,322,323,324})
            ...
            --     {
            --     startTime = 11,
            --     endTime = t1+t2+t3+t4+t5-5,
            --     cdTime = 3,
            --     leaveCd = 2,
            --     deathCd = 0,
            --     groupName = "goldedfish1", 
            --     mutex = true,    --不出同一条鱼
            --     limit = 2,
            --     }, 
            --     {
            --     fish = fishGroup["goldedfish1"],      
            --     fishTypeCount = {1}, 
    ...
    do
        ...
        -- 第二阶段场景时间划分
            ...
            -- scene.AddSwitchBgMusicEvent(t1+t2+t3, "jingbao") --警报响
            scene.AddBossWarningEvent(t1+t2+t3, FF_G.BossWarning_Purple) --预警泛紫
            scene.AddBossWarningEvent(t1+t2+t3 +2, FF_G.BossWarning_BossCome, {fishTypeId = 327})
    ...
    do
        ...
        -- 第一阶段时间划分
            ...
            for i = t1 +6, t1+t2, 115 do
                scene.AddSetPlayBgAnimEvent(i, "xunhuan") --背景动画
            end
            scene.AddSwitchBgEvent(t1+t2+30, 17)
        ...
        -- 第二阶段时间划分
            ...
            scene.AddSwitchBgMusicEvent(t1+t2+t3+2.8, "bossstage") --下个场景音效
            scene.AddSwitchBgEvent(t1+t2+t3 +4.5, 15) -- 场景
            scene.AddSwitchBgEvent(t1+t2+t3+t4+10, 18)
            scene.AddSwitchBgMusicEvent(t1+t2+t3+t4+10, "bg_02")
