...
local InitTimeline = function()
    ...
    do
        ...
        -- 第二段场景时间划分
            ...
            scene.AddHaiwanglaixiEvent(t1+t2+t3 +2, FF_G.Haiwanglaixi_FishType_KingCrab)
            -- scene.AddSwitchBgMusicEvent(t1+2.8, "fk_bg_king_02") --下个场景音效
            scene.AddSwitchBgEvent(t1+t2+t3 +4, 10) -- 场景
        ...
        -- 出小鱼单条:SetFishGroup("fish1", {0,1,2})
            ...
            for i = t1+t2+t3+8, t1+t2+t3+t3+t4, 5 do
                scene.AddFishBornEvent(i, {
                    fish = fishGroup["fish1"],      -- 指定鱼组
                    fishTypeCount = {3, 2},
                    intervalTime = 1.5,
    ...
    do
        ...
        -- 第二段场景时间划分    
            ...
            -- scene.AddSwitchBgMusicEvent(t1+t2+t3, "jingbao") --警报响
            scene.AddBossWarningEvent(t1+t2+t3, FF_G.BossWarning_Red) --预警泛紫
            scene.AddHaiwanglaixiEvent(t1+t2+t3 +2, FF_G.Haiwanglaixi_FishType_KingCrab) -- boss 预警
            scene.AddSwitchBgEvent(t1+t2+t3 +4, 10) -- 切换场景
    ...
    do
        ...
        -- 出龙虾跟随
            ...
            for i = t1+t2+t3+t4+10, t1+t2+t3+t4+t5, 45 do
                scene.AddFishBornEvent(i, {
                    fish = fishGroup["longxia"],      -- 指定鱼组
                    fishCount = {4,5},          -- 鱼的数量
                    fishTypeCount = 1,              -- 鱼类型数量
