--TODO : 需要处理 通知信息
local RechargePopup_ind = class("RechargePopup_ind", function()
    return Tools.CreateLayer("csb/RechargePopup_ind.csb")
end)

function RechargePopup_ind:onEnter()
    self:InitUI()
end

function RechargePopup_ind:OnBtnClose()
	self:Close()
end

function RechargePopup_ind:InitUI()
    ...
    Tools.AddClickEvent(btn_confirm, function()
        ...
        --     UIManager.ShowToast(TR("订单号后5位不能为空，请输入正确的单号"))
        --     return
        -- elseif #string.gsub(self.input:getText()," ", "") == 0 then
        --     print("请输入正确的单号")
        --     UIManager.ShowToast(TR("请输入正确的单号"))
        --     return
        end
...
function RechargePopup_ind:setInfo(ordernum,datelist,money,pay_type)
    if not datelist or not money or not pay_type or not ordernum then
        UIManager.ShowToast("RechargePopup_ind vip infos error")
        self:Close()
        return
    end
    if self ~= nil then
...
function RechargePopup_ind:sendRechargeInfo(rlt_)
    go(function()
