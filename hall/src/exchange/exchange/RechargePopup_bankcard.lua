local RechargePopup_bankcard = class("RechargePopup_bankcard", function()
    return Tools.CreateLayer("csb/RechargePopup_bankcard.csb")
end)

function RechargePopup_bankcard:onEnter()
    self:InitUI()

    if ConfigParam.Region == "mm" then
        self:MakeLangMM()
    end
end

function RechargePopup_bankcard:MakeLangMM()
    -- 银行用户名
    ...
    -- 付款账号
    local _lang_bankcard = self:findChild("_lang_bankcard")
    _lang_bankcard:setString(TR("转账ID"))
...
function RechargePopup_bankcard:OnBtnClose()
	self:Close()
end

function RechargePopup_bankcard:InitUI()
    ...
    Tools.AddClickEvent(btn_confirm, function()
        if self.input:getText() == "" or exutils.strIsBlank(self.input:getText()) then
            UIManager.ShowToast(TR("请输入正确的付款账号"))
            return
        end
...
function RechargePopup_bankcard:setInfo(ordernum,datelist,money,pay_type)
    if not datelist or not money or not pay_type or not ordernum then
        UIManager.ShowToast("RechargePopup_bankcard vip infos error")
        self:Close()
        return
    end
    if self ~= nil then
...
function RechargePopup_bankcard:sendRechargeInfo(rlt_)
    go(function()
