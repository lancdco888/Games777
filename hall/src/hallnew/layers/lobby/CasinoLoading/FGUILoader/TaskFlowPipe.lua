...
function TaskFlowPipe:update(delta)
	...
	if not ret then
		print("TASK ERROR BEGIN **********************")
		print(msg)
		print("TASK ERROR END   **********************")
...
function TaskFlowPipe:_updatePercent()
	...
	for k,task in pairs(self.tasks) do
		if self.executeCount > k - 1 then
			cur = cur + task.progressWeight * task.weightScale
		elseif self.executeCount == k - 1 then
			cur = cur + math.min(task.progressWeight, task.curProgress) * task.weightScale
		end
		total = total + task.progressWeight * task.weightScale
	end

	-- 整体总进度(按任务进度权重来算，可视为总体真实进度)
	local totalPercent = 1
	if total > 0 then
		totalPercent = cur / total
	end

	self.onProcessCallback(taskPercent, totalPercent)
end
