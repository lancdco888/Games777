...
CodeGen_744_CoinTiger_md5 ="#*MD5<57075571e309e54f68b9685c8763ea91>*#"

--[[
Sugar请求免费旋转图形
]]
PKG_CoinTiger_FreeSpin = {
    typeName = "PKG_CoinTiger_FreeSpin",
    typeId = 43741,
    Create = function(o)
        if o == nil then
            o = {}
            setmetatable(o, PKG_CoinTiger_FreeSpin)
        end
        o.betMoney = 0 -- Int64
        ...
        o.grids = {} -- List<PKG.CoinTiger.Cell>
        o.lines = {} -- List<PKG.CoinTiger.Line>
        o.curWinCoin = 0 -- Int64
        o.totalWinCoin = 0 -- Int64
        o.intoCollect = 0 -- Int32
        o.baseScale = 0 -- Int32
        o.totalScale = 0 -- Int32
        o.restTimes = 0 -- Int32
        o.totalFreeTimes = 0 -- Int32
        o.addFreeTimes = 0 -- Int32
        o.FreeMode = 0 -- Int32
        return o
    end,
    Read = function(self, om)
        ...
        self.grids = o
        for i = 1, len do
            o[i] = PKG_CoinTiger_Cell.Create(); r = o[i]:Read(om)
            if r ~= 0 then return r end
        end
        if r ~= 0 then return r end
        -- lines
        ...
        self.lines = o
        for i = 1, len do
            o[i] = PKG_CoinTiger_Line.Create(); r = o[i]:Read(om)
            if r ~= 0 then return r end
        end
        if r ~= 0 then return r end
        -- curWinCoin
        ...
        if r ~= 0 then return r end
        -- baseScale
        r, self.baseScale = d:Rvi32()
        if r ~= 0 then return r end
        -- totalScale
        r, self.totalScale = d:Rvi32()
        if r ~= 0 then return r end
        -- restTimes
        r, self.restTimes = d:Rvi32()
        if r ~= 0 then return r end
        -- totalFreeTimes
        r, self.totalFreeTimes = d:Rvi32()
        if r ~= 0 then return r end
        -- addFreeTimes
        r, self.addFreeTimes = d:Rvi32()
        if r ~= 0 then return r end
        -- FreeMode
        r, self.FreeMode = d:Rvi32()
        if r ~= 0 then return r end
        return 0
    end,
    Write = function(self, om)
        ...
        -- baseScale
        d:Wvi32(self.baseScale)
        -- totalScale
        d:Wvi32(self.totalScale)
        -- restTimes
        d:Wvi32(self.restTimes)
        -- totalFreeTimes
        d:Wvi32(self.totalFreeTimes)
        -- addFreeTimes
        d:Wvi32(self.addFreeTimes)
        -- FreeMode
        d:Wvi32(self.FreeMode)
    end
}
PKG_CoinTiger_FreeSpin.__index = PKG_CoinTiger_FreeSpin

--[[
Sugar请求特殊旋转图形
]]
PKG_CoinTiger_NormalSpin = {
    typeName = "PKG_CoinTiger_NormalSpin",
    typeId = 43740,
    Create = function(o)
        if o == nil then
            o = {}
            setmetatable(o, PKG_CoinTiger_NormalSpin)
        end
        o.betMoney = 0 -- Int64
        o.grids = {} -- List<PKG.CoinTiger.Cell>
        o.lines = {} -- List<PKG.CoinTiger.Line>
...
PKG_CoinTiger_NormalSpin.__index = PKG_CoinTiger_NormalSpin

--[[
普通结果
]]
PKG_Slots_Client_CoinTigerNormalRet = {
    typeName = "PKG_Slots_Client_CoinTigerNormalRet",
    typeId = 43744,
    Create = function(o)
        if o == nil then
            o = {}
            setmetatable(o, PKG_Slots_Client_CoinTigerNormalRet)
        end
        o.normalSpin = PKG_CoinTiger_NormalSpin.Create() -- PKG.CoinTiger.NormalSpin
        return o
    end,
    Read = function(self, om)
        local d = om.d
        local r, n
        -- normalSpin
        self.normalSpin = PKG_CoinTiger_NormalSpin.Create(); r = self.normalSpin:Read(om)
        if r ~= 0 then return r end
        return 0
    end,
    Write = function(self, om)
...
PKG_Slots_Client_CoinTigerNormalRet.__index = PKG_Slots_Client_CoinTigerNormalRet

--[[
特殊结果
]]
PKG_Slots_Client_CoinTigerFreeRet = {
    typeName = "PKG_Slots_Client_CoinTigerFreeRet",
    typeId = 43745,
    Create = function(o)
        if o == nil then
            o = {}
            setmetatable(o, PKG_Slots_Client_CoinTigerFreeRet)
        end
        o.freeSpin = PKG_CoinTiger_FreeSpin.Create() -- PKG.CoinTiger.FreeSpin
        return o
    end,
    Read = function(self, om)
        local d = om.d
        local r, n
        -- freeSpin
        self.freeSpin = PKG_CoinTiger_FreeSpin.Create(); r = self.freeSpin:Read(om)
        if r ~= 0 then return r end
        return 0
    end,
    Write = function(self, om)
...
PKG_Slots_Client_CoinTigerFreeRet.__index = PKG_Slots_Client_CoinTigerFreeRet
...
PKG_Client_Slots_CoinTigerNormalSpin = {
    typeName = "PKG_Client_Slots_CoinTigerNormalSpin",
    typeId = 43742,
    Create = function(o)
        if o == nil then
            o = {}
            setmetatable(o, PKG_Client_Slots_CoinTigerNormalSpin)
        end
        --[[
        押注金额
        ]]
...
PKG_Client_Slots_CoinTigerNormalSpin.__index = PKG_Client_Slots_CoinTigerNormalSpin

--[[
断线重连数据恢复
]]
PKG_Slots_Client_CoinTigerEnterResumed = {
    typeName = "PKG_Slots_Client_CoinTigerEnterResumed",
    typeId = 43746,
    Create = function(o)
        if o == nil then
            o = {}
            setmetatable(o, PKG_Slots_Client_CoinTigerEnterResumed)
        end
        --[[
        进入游戏的常规数据恢复
        ]]
        ...
        o.cells = {} -- List<Int32>
        o.resumedNormal = PKG_Slots_Client_CoinTigerNormalRet.Create() -- PKG.Slots_Client.CoinTigerNormalRet
        o.resumedFree = PKG_Slots_Client_CoinTigerFreeRet.Create() -- PKG.Slots_Client.CoinTigerFreeRet
    ...
    Read = function(self, om)
        ...
        if r ~= 0 then return r end
        -- resumedNormal
        self.resumedNormal = PKG_Slots_Client_CoinTigerNormalRet.Create(); r = self.resumedNormal:Read(om)
        if r ~= 0 then return r end
        -- resumedFree
        self.resumedFree = PKG_Slots_Client_CoinTigerFreeRet.Create(); r = self.resumedFree:Read(om)
        if r ~= 0 then return r end
        -- lastMsgType
...
PKG_Slots_Client_CoinTigerEnterResumed.__index = PKG_Slots_Client_CoinTigerEnterResumed
...
PKG_Client_Slots_CoinTigerFreeSpin = {
    typeName = "PKG_Client_Slots_CoinTigerFreeSpin",
    typeId = 43743,
    Create = function(o)
        if o == nil then
            o = {}
            setmetatable(o, PKG_Client_Slots_CoinTigerFreeSpin)
        end
        return o
    end,
    Read = function(self, om)
...
PKG_Client_Slots_CoinTigerFreeSpin.__index = PKG_Client_Slots_CoinTigerFreeSpin
...
PKG_CoinTiger_Cell = {
    typeName = "PKG_CoinTiger_Cell",
    Create = function(o)
        if o == nil then
            o = {}
            setmetatable(o, PKG_CoinTiger_Cell)
        end
        --[[
        点对应下标
        ]]
    ...
    Read = function(self, om)
        ...
        if r ~= 0 then return r end
        -- bonusType
        r, self.bonusType = d:Rvi32()
        if r ~= 0 then return r end
        -- bonusValue
        r, self.bonusValue = d:Rvi64()
        if r ~= 0 then return r end
        return 0
    end,
    Write = function(self, om)
        ...
        d:Wvi32(self.icon)
        -- bonusType
        d:Wvi32(self.bonusType)
        -- bonusValue
        d:Wvi64(self.bonusValue)
    end
}
PKG_CoinTiger_Cell.__index = PKG_CoinTiger_Cell

PKG_CoinTiger_Line = {
    typeName = "PKG_CoinTiger_Line",
    Create = function(o)
        if o == nil then
            o = {}
            setmetatable(o, PKG_CoinTiger_Line)
        end
        --[[
        中奖线(eg:1号线)
        ]]
        ...
        o.lineCells = {} -- List<PKG.CoinTiger.Cell>
        return o
    end,
    Read = function(self, om)
        local d = om.d
...
PKG_CoinTiger_Line.__index = PKG_CoinTiger_Line
...
local o = ObjMgr
o.Register(PKG_CoinTiger_FreeSpin)
o.Register(PKG_CoinTiger_NormalSpin)
o.Register(PKG_Slots_Client_CoinTigerNormalRet)
o.Register(PKG_Slots_Client_CoinTigerFreeRet)
o.Register(PKG_Slots_Client_Enter_Success)
o.Register(PKG_Slots_Client_SyncPlayerStates)
o.Register(PKG_Client_Slots_CoinTigerNormalSpin)
o.Register(PKG_Slots_Client_CoinTigerEnterResumed)
...
o.Register(PKG_Client_Slots_CoinTigerFreeSpin)