...
local AdjustSDK = require("hall.src.sdk.AdjustSDK")

function Sdk:ctor()
    require("hall.src.sdk.MetaPixel")
    ...
    if apiKey ~=nil and apiKey ~= "" and secret ~= nil  and secret ~= "" then
        self.singular = require("hall.src.sdk.Singular").new()
        self.singular:initSDK(apiKey, secret)
    else
        self.singular = nil
    end

	self.firebase = require("hall.src.sdk.FireBase").new()

    self.adjustSdk = AdjustSDK.new()
end

function Sdk:unload()
	for k,v in pairs(package.loaded) do
		if string.find(k, "hall.src.sdk.") == 1 then
...
function Sdk:registerCompleted()
	...
	if Device.PushFacebookEvent then
		Device:PushFacebookEvent("1")
	end

    self.adjustSdk:PushAdjustEvent(AdjustSDK.EventName.CompleteRegistration)
end
...
function Sdk:purchase(amount)
	...
	if Device.PushFacebookEvent then
		Device:PushFacebookEvent("2")
	end

    self.adjustSdk:PushAdjustEvent(AdjustSDK.EventName.Purchase)
end
...
function Sdk:firstOpen()
    ...
    self.adjustSdk:PushAdjustEvent(AdjustSDK.EventName.StartTrial)
end
