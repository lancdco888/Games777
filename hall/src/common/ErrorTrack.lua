...
local function ReadTail(filename, bytes)
    ...
    local start = math.max(0, size - bytes)
...
function LuaErrorReportSend(errMsg, callback, debugModeDisableReporting, errMsgDetails)
    ...
    local logPath = get_log_path()
    local logTails = ReadTail(logPath, 16 * 1024)
    local logs = logTails or "logs is empty"
    logs = logs .. errMsg

    local data = {
        log_type = "error_20",
        message = "@@ WITH_LOG @@" .. errMsg,
        user = "no login",
