...
function Reel:RandomSymbolData()
    local data = {
        icon  = math.random(1, 13),
        type = 0
    }
    --第一列或者第五列出现ssc替换图标
    if data.icon == 1 and (self.idx == 1 or self.idx == 5)  then
        data.icon = math.random(3,13)
    end
    --在免费游戏中wild图标颜色变化
    if data.icon == 1 and Reel.freeicotype ~= 0 then
        data.icon = Reel.freeicotype 
    end
    return data
end
...
function Reel:OnCreateSymbolRenderComponent()
   return FairyGUI.UIPackage.CreateObject("Game465", "Symbol")
end
...
function Reel:UpdateSymbol(render, data, index)
    ...
    local icon = cfg.icon
    if  data.icon == 1 and Reel.freeicotype ~= 0 then
        icon = SymbolConfig[Reel.freeicotype].icon
    end
    loader.url = icon
