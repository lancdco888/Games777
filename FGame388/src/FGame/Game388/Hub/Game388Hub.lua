...
---@class Game388Hub extends GameProtocolHub
local cls = class("Game388Hub", super)
...
function cls:parseNormalSpinRet(aData)
    ...
    local ret = {
        _msgName_ = "PB.Slots_Client.NvXiaNormalRet",
        normalSpin = {
            bet = aData.bet,
            winCoin = aData.win,
    ...
    if aData.is_into_special then
        ret.normalSpin.intoSpecial = aData.special_total_count or 9
    end

    if aData.is_into_free then
        ret.normalSpin.intoFree = aData.free_total_count or 8
    end
...
function cls:parseFreeSpinRet(aData)
    ...
    local ret = {
        _msgName_ = "PB.Slots_Client.NvXiaFreeRet",
        freeSpin = {
            bet = 0, --aData.bet,
            winCoin = aData.win,
    ...
    if aData.is_into_special then
        ret.freeSpin.intoSpecial = aData.special_total_count or 9
    end
...
function cls:parseSpecialData(aData)
    ...
    local ret = {
        _msgName_ = "PB.Slots_Client.NvXiaSpecialRet",
        specialSpin = {
            bet = 0,
            lines = {},
            ...
            winCoin = aData.total_special_win,
            allCount = aData.total_count,
            levelUpMoney = aData.vip_level,
            levelUpMoneys = aData.level_ups,
