...
function SpecialPanel:EnterSpecial(winnumber,overDate,callback,cells)
    --FToolSet.PlayFGUISound("ui://Game479/202308151127(21)")
    self.game:ChangeBgm("ui://Game479/SND_BACKGROUND_THEME")
...
function SpecialPanel:OnSelectMode(index,isauto)
    ...
    if not isauto then
        ...
        FToolSet.PlayFGUISound(string.format("ui://Game479/SND_REVEAL_%d",typeNum))
        --FToolSet.PlayFGUISound(MusicCfg.jackpot_hit)
        self:PushCoinInfo(index)
    else
        FToolSet.PlayFGUISound("ui://Game479/SND_drumroll")
    end
    self:StopAutoClickTimer()
    ...
    if self.playedindex >= #self.overDate then
        ...
        self.game:ChangeBgm()
        FToolSet.PlayFGUISound("ui://Game479/SND_ST_End_Tag")
        StartOnceTimer(function ()
        ...
        StartOnceTimer(function ()
            local music_str = string.format("ui://Game479/SND_ST_%s_479",animtype[endtype])
            ...
            self:HideList(self.btnlist)
            local caitype = {}
            self.endwintype.url = string.format("ui://Game479/winend(%d)_479",5-endtype)
