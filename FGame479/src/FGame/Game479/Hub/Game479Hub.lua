...
---@class Game479Hub : GameProtocolHub
local Game479Hub = class("Game479Hub", super)

function Game479Hub:parseNormalSpinRet(aData,collect_indexes)
    local ret = { normalSpin = {} }
    dump(aData, "parseNormalSpinRet", 10)
    ret._msgName_ = "PB.Slots_Client.PandaTreasuresNormalRet"
    ...
    -- dump(ret,">>>>>>>>>>>>>>>>>>>>>Game479NormalSpinRes")
    return ret;
end
function Game479Hub:parseFreeSpinType(aData,isrecover)
    ...
    local ret = {}
    ret._msgName_ = "PB.Slots_Client.PandaTreasuresFreeRetType"
    ret.freeType = { 
...
function Game479Hub:parseFreeSpinRet(aData,collect_indexes)
    dump(aData,"Game479Hub:parseFreeSpinRet:aData",10)
    local ret = {}
    local spinret = {}
    ret._msgName_ = "PB.Slots_Client.PandaTreasuresFreeRet"
    spinret.bet = 0;
...
--判断是不是正常模式
function Game479Hub:IsNormalState(last_state)
    if type(last_state) == "string" and last_state == "Normal" then
        return true
    end
    return false
end
function Game479Hub:parseResumeData(aEnter, aEnterData, aResume)
     dump(aResume,"Game479Hub:parseResumeData::aResume",6)
         dump(aEnter,"Game479Hub:parseResumeData::aEnter",6)
    -- dump(aEnterData,"Game479Hub:parseResumeData::aEnterData",3)
...
--判断免费情况中的特殊情况
function Game479Hub:checkFreeState(currData,LastData,resume)
    print("判断免费情况中的特殊情况")
    --设置上一轮金币
...
function Game479Hub:parseGrids(aData)
    local result = {}
...
function Game479Hub:parseGrid(aData)
    return {
        index = aData.index,
        icon = aData.icon,
    }
