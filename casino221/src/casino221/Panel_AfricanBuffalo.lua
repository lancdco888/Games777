...
local AfricanBuffalo_Item = require "casino221.src.casino221.AfricanBuffalo_Item"
...
this.getAnim_ByIcon = function(icon,index)
	...
	if icon == AfricanBuffalo_Item.iconData.wild_icon then
		anim = sp.SkeletonAnimation:createWithJsonFile("casino221/newres/spine/Wild.json","casino221/newres/spine/Wild.atlas",1)
	...
	elseif icon == AfricanBuffalo_Item.iconData.scatter_icon then
		anim = sp.SkeletonAnimation:createWithJsonFile("casino221/newres/spine/Scatter.json","casino221/newres/spine/Scatter.atlas",1)
		anim:setOpacityModifyRGB(true)
		anim:setAnimation(0,"Scatter_Loop",true)
	elseif icon == AfricanBuffalo_Item.iconData.ox_icon then
		anim = sp.SkeletonAnimation:createWithJsonFile("casino221/newres/spine/H1.json","casino221/newres/spine/H1.atlas",1)
		anim:setOpacityModifyRGB(true)
...
this.getAnim_EffectFrame = function()
	local effectFrame = sp.SkeletonAnimation:createWithJsonFile("casino221/newres/spine/YingFenKuang.json","casino221/newres/spine/YingFenKuang.atlas",1)
			...
			if time >= 3 then
				for j=1,#resultLine[i].lineCells do
					...
					if icon == AfricanBuffalo_Item.iconData.ox_icon then
						local spriteTem = sp.SkeletonAnimation:createWithJsonFile("casino221/newres/spine/H1.json","casino221/newres/spine/H1.atlas",1)
					...
					if icon == AfricanBuffalo_Item.iconData.wild_icon then
						local spriteTem = sp.SkeletonAnimation:createWithJsonFile("casino221/newres/spine/Wild.json","casino221/newres/spine/Wild.atlas",1)
						spriteTem:setOpacityModifyRGB(true)
...
this.HandleCasinoSpin_Normal = function()
	...
	print("收到casino221_Normal的回包,开始读取数据...")
...
this.HandleCasinoSpin_Free = function()
	dump(this.logic.rcvSpinData)
	print("收到casino221_Free的回包,开始读取数据...")
...
this.createFreeUI = function(usedNum,totalNum)
	...
	this.upFreegame_Text = cc.Label:createWithBMFont("casino221/newres/fnt/fg_num.fnt",text,0)
...
this.CreateFreePanel = function(panelType,num,totalwin)
	...
	elseif panelType == 3 then
		...
		local freeText = cc.Label:createWithBMFont("casino221/newres/fnt/total_num.fnt",Casino_Func.ShuZi_Exchangerate(totalwin,false),0)
