...
function BaseSlot:ctor(parent,game)    
    ...
    -- 中层转轴容器(mask遮罩用)
    self.middleContainer = FairyGUI.GComponent()
    self.middleContainer.width = parent.width
    self.middleContainer.height = parent.height
    parent:AddChild(self.middleContainer)
...
function BaseSlot:__delete()
    ...
    self.middleContainer:RemoveFromParent(true)
...
function BaseSlot:SetVisible(value)
    ...
    self.middleContainer.visible = value
    self.topContainer.visible = value
end

function BaseSlot:GetOrCreateTopSymbol(topzorder,index,grid_str)
    ...
    if not render then
        ...
        render = FairyGUI.UIPackage.CreateObject("Game375", "Symbol_" .. grid_str)
        render:SetPivot(0.5, 0.5, true)
        render.x = (logicX ) * (reelCfg.reelWidth + reelCfg.reelSpace) + 0.5 * reelCfg.reelWidth + reelCfg.offx
        render.y = (logicY + 0.5) * (reelCfg.reelHeight / reelCfg.yCellNumber) + reelCfg.offy
        self.topContainer:AddChild(render)
        self.topSymbolRenders[index] = render
    end

    render.sortingOrder = topzorder + index
    render.visible = true

    return render
end

function BaseSlot:GetOrCreateMiddleSymbol(topzorder,index,grid_str)
    local render = self.middleSymbolRenders[index]
    if not render then
        local reelCfg = self._reelcfg
        ...
        self.middleContainer:AddChild(render)
        self.middleSymbolRenders[index] = render
    end
...
function BaseSlot:RemoveTopSymbol()
    ...
    self.topSymbolRenders = {}
    for k, render in pairs(self.middleSymbolRenders) do
        render:RemoveFromParent(true)
        FTween.KillTweens(render)
    end
    self.middleSymbolRenders = {}
...
-- @brief 获取顶层格子表
function BaseSlot:GetTopSymbolList()
    return clone(self.topSymbolRenders)
end
